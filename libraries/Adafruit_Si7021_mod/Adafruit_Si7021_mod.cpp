...
#include "Arduino.h"
#include <Adafruit_Si7021_mod.h>
#include <Wire.h>

/*!
 *  @brief  Instantiates a new Adafruit_Si7021_mod class
 *  @param  *theWire
 *          optional wire object
 */
Adafruit_Si7021_mod::Adafruit_Si7021_mod(TwoWire *theWire) {
 ...
 */
bool Adafruit_Si7021_mod::begin() {
  _wire->begin();
...
bool Adafruit_Si7021_mod::init() {
    return begin();
}
void Adafruit_Si7021_mod::measure() {
    _temperature=readTemperature();
    _humidity=readHumidity();
}
float Adafruit_Si7021_mod::temperature(){
    return _temperature;
}
float Adafruit_Si7021_mod::humidity(){
    return _humidity;
}
 ...
 */
float Adafruit_Si7021_mod::readHumidity() {
  _wire->beginTransmission(_i2caddr);
 ...
 * timeout
 */
float Adafruit_Si7021_mod::readTemperature() {
  _wire->beginTransmission(_i2caddr);
 ...
 */
void Adafruit_Si7021_mod::reset() {
  _wire->beginTransmission(_i2caddr);
...
void Adafruit_Si7021_mod::_readRevision(void) {
  _wire->beginTransmission(_i2caddr);
 ...
 */
void Adafruit_Si7021_mod::readSerialNumber() {
  _wire->beginTransmission(_i2caddr);
 ...
 *  @return model value
 */
si_sensorType Adafruit_Si7021_mod::getModel() { return _model; }

/*!
 *  @brief  Enable/Disable sensor heater
 */
void Adafruit_Si7021_mod::heater(bool h) {
 ...
 */
bool Adafruit_Si7021_mod::isHeaterEnabled() {
  uint8_t regValue = _readRegister8(SI7021_READRHT_REG_CMD);
 ...
 */
void Adafruit_Si7021_mod::setHeatLevel(uint8_t level) {
  _writeRegister8(SI7021_WRITEHEATER_REG_CMD, level);
}

/*******************************************************************/

void Adafruit_Si7021_mod::_writeRegister8(uint8_t reg, uint8_t value) {
  _wire->beginTransmission(_i2caddr);
  _wire->write(reg);
  _wire->write(value);
  _wire->endTransmission();
...
uint8_t Adafruit_Si7021_mod::_readRegister8(uint8_t reg) {
  uint8_t value;
  _wire->beginTransmission(_i2caddr);
...
uint16_t Adafruit_Si7021_mod::_readRegister16(uint8_t reg) {
  uint16_t value;
  _wire->beginTransmission(_i2caddr);
  _wire->write(reg);
